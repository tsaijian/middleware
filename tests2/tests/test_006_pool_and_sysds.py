...
def ws_client(api_config):
    # by the time this test is called in the pipeline,
    # the HA VM should have networking configured so
    # we can use the VIP
    ip = api_config['ip1'] if not api_config['is_ha'] else api_config['vip']
    with client(host_ip=ip, passwd=api_config['password']) as c:
        yield c
...
def test_002_create_permanent_zpool(request, api_config, ws_client):
    """
    ...
    pool_name = api_config['pool_name']
    try:
        ws_client.call(
            'pool.create', {
                'name': pool_name,
...
def test_004_verify_pool_property_unused_disk_functionality(request, api_config, ws_client, pool_data):
    """
