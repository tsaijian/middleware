...
class UserService(CRUDService):
    ...
    async def query(self, filters, options):
        ...
        now = datetime.utcnow()
        for entry in result:
            if entry['last_password_change'] not in (None, 0):
                entry['password_age'] = (now - entry['last_password_change']).days
            else:
                entry['password_age'] = None
    ...
    async def reset_password(self, app, old_password, new_password):
        ...
        if entry['password_aging_enabled']:
            if (datetime.utcnow() - entry['last_password_change']).days < (entry['min_password_age'] or 0):
